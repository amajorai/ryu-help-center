		...
		<div className="flex min-h-0 flex-1 items-center justify-center p-6">
			<div className="max-w-md rounded-3xl border border-border/70 bg-card p-8 text-center shadow-sm">
				...
				<p className="mt-3 text-muted-foreground text-sm leading-relaxed">
					Create a ticket or load sample data to inspect the support workflow.
				</p>
				...
				{error ? (
					<p className="mt-4 text-status-destructive text-xs" role="alert">
						{error}
					</p>
				) : null}
			</div>
	...
	return (
		...
		>
			<main className="flex min-w-0 flex-1 flex-col">
				{mode === "demo" ? (
					<div
						className="border-border/70 border-b bg-muted/30 px-5 py-2 font-medium text-muted-foreground text-xs"
						data-testid="help-center-demo-banner"
						role="status"
					>
						Demo mode · sample records are local and are not customer data.
					</div>
				) : null}
				{mode === "loading" ? (
